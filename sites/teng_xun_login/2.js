...
pt.plogin = {
	...
	dottedShow: function() {
		try {
			...
			var f = c.getElementsByTagName("span");
			if (!f || f.length == 0) {
				return
			}
			var h = f[f.length - 1];
			...
			var d = false;
			while (g) {
				var a = g.tagName && g.tagName.toLowerCase();
				if (a == "a") {
					d = true;
					break
				}
			...
			$.report.nlog("dotted show " + j.message)
		}
	},
	...
	onekeyVerify: function(b, c, m) {
		var a = $("onekey_verify"),
			j = $("ov_retry_wrap"),
			q = $("ov_retry"),
			l = $("ov_back"),
			d = $("p"),
			r = document.loginform;
		if (!a) {
			return
		}
		...
		var f = a.getElementsByTagName("span");
		var o = ["占位"];
		for (var g = 0; g < f.length; g++) {
			if (f[g].className.indexOf("ov-tips") > -1) {
				o.push(f[g])
			}
		}
		q.onclick = function() {
		...
		function k() {
			...
			m = m || pt.plogin.onekeyVerify.__tips || "您的帐号千金难求。为确保安全，请务必对手机收到的登录请求进行确认。";
			pt.plogin.onekeyVerify.__tips = m;
			pt.plogin.show_err(m, true);
